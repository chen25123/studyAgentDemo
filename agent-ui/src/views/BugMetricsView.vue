...
onMounted(async () => {
  ...
  try {
    ...
    metrics.value = data.filter((m) => m.entity_code === "bug");
...
<template>
  <section class="workspace" aria-label="Bug 质量">
    <header class="topbar">
      <div>
        <p class="eyebrow">Bug 质量</p>
        <h2>Bug 指标本月实时数据</h2>
      </div>
    </header>
    ...
    <p v-if="!loading && metrics.length === 0" class="empty-hint">暂无 Bug 指标</p>
  </section>
</template>
